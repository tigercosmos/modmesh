...
public:
    ...
    /// carried by more than one channel appears once, named by the schema of
    /// the last channel.
    std::map<std::string, std::string> topics() const;

    /// Log time of the first and of the last message, in nanoseconds.  A
    /// summary that states neither a statistics record nor a chunk index
    /// leaves the range at (0, 0); has_time_range() tells the two apart.
...
private:
    ...
    void read_summary();
    void parse_summary(std::string_view records);
