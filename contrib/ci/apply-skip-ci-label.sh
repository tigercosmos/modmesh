...
for workflow in $WORKFLOWS; do
  ...
  for id in $(runs "$workflow" \
      '.workflow_runs[] | select(.status == "completed") | .id'); do
    # Re-run only a run that stopped short. A run that built the commit for
    # real keeps its result rather than spending the matrix again. A path
    # gate stops a run the same way, and re-running one of those costs
    # seconds, because the gate reaches the same answer again.
